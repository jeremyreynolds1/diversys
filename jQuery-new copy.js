...
function animate(animationName, animationDuration, delay, numImages){
	...
	var uppercaseBegin = 66;
	var count = 0;
	//dynamically creating links for images.
	while(count < numImages){
		arrayLetters[count] = String.fromCharCode(lowercaseBegin);
			//extra if statement in case alphabet is exhausted.
			//format will now be mod_XX_topic_XX_scene_XX_aB
			if(count > 25){
				lowercaseBegin = 98;
	...
	for(var i=0; i<finalPath.length; i++){
		//create each "image" as an object containing certain variables.
		var image = {
	...
	var nextImage = 2;
		...
		function change(imageArray){
			setTimeout(function(){
				...
				$("#image"+ nextImage ).addClass("animated "+ imageArray[count].animationName);
				currentImage++;
				nextImage++
				count++;
				//delay += 5000;
